...
BEGIN {
    ...
    my @eth_types = qw(
        ETH_TYPE_IP ETH_TYPE_IPv4 ETH_TYPE_ARP ETH_TYPE_IPv6
    );
    my @eth_addr = qw(
    ...
    my @constants  = ( @eth_types, @eth_addr, @ip_addr, @arp );
    ...
    # Make "decode_ip" an alias of decode_ipv4();
    *decode_ip = \&decode_ipv4;
}
...
use constant {
    ...
    ARP_PROTO_IPv4      => ETH_TYPE_IPv4,

    # Hex strings representing special address values.
    ETH_ADDR_BROADCAST  => 'ff' x ARP_HLEN_ETHERNET,
    IPv4_ADDR_BROADCAST => 'ff' x ARP_PLEN_IPv4,
    ETH_ADDR_NONE       => '00' x ARP_HLEN_ETHERNET,
    IPv4_ADDR_NONE      => '00' x ARP_PLEN_IPv4,
};
...
=head1 SYNOPSIS
 ...
 if ( $eth_data->{type} == ETH_TYPE_IPv4 ) {
    ...
    printf( "%s -> %s, %d octets (including IP header)\n",
...
The constants below can be imported individually or by using appropriate tags.
...
Tags: C<all>, C<const>, C<eth_types>.
...
Ethernet protocol address length in octets (6).
...
IP protocol address length in octets (4).

=back

=head1 FUNCTIONS

The functions below can be imported individually, or by using the C<:func> or C<:all> tags:

  use M6::ARPSponge::NetPacket qw( :all );
  use M6::ARPSponge::NetPacket qw( :func );

All functions return a hash ref (not an object!) with only a minimal set of fields
set. B<They do not set C<_parent> or C<_frame>!>
